...
const pointer_start_x = 0.7
const pointer_start_y = 0.5
...
func _process(_delta):
	...
	if (contract_state == 0):
		...
		if (local_in.has_point(pointer_node.position)):
			var thresholdContinueScroll = get_viewport_rect().size.y - scroll_next_offset
			if (pointer_node.position.y >= thresholdContinueScroll):
				var pos_y_new = pos_contract.y - 100
