...
const double VideoHandDetector::TICK_FREQUENCY = cv::getTickFrequency();

// VideoHandDetector::VideoHandDetector(const std::string cascadeFilePath, cv::VideoCapture &videoCapture)
// {
...
void VideoHandDetector::setVideoCapture(cv::VideoCapture &videoCapture)
{
    m_videoCapture = &videoCapture;
}

cv::VideoCapture *VideoHandDetector::videoCapture() const
{
    return m_videoCapture;
}

void VideoHandDetector::setFaceCascade(const std::string cascadeFilePath)
{
...
cv::CascadeClassifier *VideoHandDetector::faceCascade() const
{
    return m_faceCascade;
}

std::queue<cv::Mat> VideoHandDetector::getLastKnownFaceTemplateQueue() const
{
    return m_faceTemplate_lastKnown_queue;
}

void VideoHandDetector::setResizedWidth(const int width)
{
    m_resizedWidth = std::max(width, 1);
}

int VideoHandDetector::resizedWidth() const
{
    return m_resizedWidth;
}

bool VideoHandDetector::isFaceFound() const
{
	return m_foundFace;
}

cv::Rect VideoHandDetector::face() const
{
...
cv::Point VideoHandDetector::facePosition() const
{
    cv::Point facePos;
...
void VideoHandDetector::setTemplateMatchingMaxDuration(const double s)
{
    m_templateMatchingMaxDuration = s;
}

double VideoHandDetector::templateMatchingMaxDuration() const
{
...
cv::Rect VideoHandDetector::doubleRectSize(const cv::Rect &inputRect, const cv::Rect &frameSize) const
{
    cv::Rect outputRect;
    // Double rect size
...
cv::Point VideoHandDetector::centerOfRect(const cv::Rect &rect) const
{
    return cv::Point(rect.x + rect.width / 2, rect.y + rect.height / 2);
}

cv::Rect VideoHandDetector::biggestFace(std::vector<cv::Rect> &faces) const
{
    assert(!faces.empty());
...
*/
cv::Mat VideoHandDetector::getFaceTemplate(const cv::Mat &frame, cv::Rect face)
{
    face.x += face.width / 4;
...
void VideoHandDetector::detectFaceAllSizes(const cv::Mat &frame)
{
...
void VideoHandDetector::detectFaceAroundRoi(const cv::Mat &frame)
{
...
void VideoHandDetector::detectFacesTemplateMatching(const cv::Mat &frame, const cv::Mat &orgFrame)
{
...
cv::Point VideoHandDetector::getFrameAndDetect(cv::Mat &frame)
{
...
cv::Point VideoHandDetector::operator>>(cv::Mat &frame)
{
